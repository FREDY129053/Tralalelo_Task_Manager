...
function OverlayTask({ task }: { task: ITask }) {
  ...
  return (
    <div
      ...
      className="border border-[#111012]/10 w-full text-sm leading-8 items-center rounded-lg p-3 z-10 cursor-grabbing"
    >
      <div className="flex flex-col">
        <div className="flex flex-col">
          <span className="text-gray-500">
            {day} {abstractMonth[month]}
          </span>
          ...
          <div className="flex flex-row gap-2">
            <span>
              {task.total_comments > 0 ? (
                <LiaComment className="text-xl text-gray-500" />
              ) : (
                <></>
              )}
            </span>
            ...
            (task.total_subtasks < 21 ? (
              <div className="flex flex-1 gap-[2px]">
                {Array.from({ length: task.total_subtasks }).map((_, idx) => (
                  <div
                    key={idx}
                    className={`h-2 rounded-sm flex-1 min-w-[4px] ${
                      idx < task.completed_subtasks
                        ? "bg-green-500"
                        : "bg-gray-300"
                    }`}
                  />
...
export default React.memo(OverlayTask);
